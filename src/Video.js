...
const server_url = process.env.NODE_ENV === 'production' ? 'https://wecall-test-app.herokuapp.com' : "http://localhost:4001"
...
const Video = () =>  {
  ...
  const getPermissions = async () => {
    try {
      ...
      if (navigator.mediaDevices.getDisplayMedia) {
        setState({ screenAvailable: true });
      } else {
        setState({ screenAvailable: false });
      }
  ...
  const openChat = () => setState({ showModal: true, newmessages: 0 });
  const closeChat = () => setState({ showModal: false });
  const handleMessage = (e) => setState({ message: e.target.value });
  ...
  const handleUsername = (e) => {console.log("username set"); setState({ username: e.target.value })};
  ...
  const connect = () =>
    setState({ askForUsername: false }, () => {console.log("Username Added");getMedia()});
